...
本プロジェクトには 3 つの GitHub Actions ワークフローが含まれています：

1. **Weekly Menu Generation** (`weekly-menu.yml`)
   - **実行スケジュール**: 毎週土曜日 06:00 JST（本番環境）
   - **処理内容**: 次週の献立を自動生成してSlackに通知
   - **手動実行**: 可能（Actions タブから実行可能）

2. **Daily Reminder** (`daily-reminder.yml`)
   - **実行スケジュール**: 毎日 19:00 JST（本番環境）
   - **処理内容**: 今日の献立と実績入力のリマインドをSlackに送信
   - **手動実行**: 可能（Actions タブから実行可能）

3. **Test Full Cycle** (`test-full-cycle.yml`) 🧪
   - **実行スケジュール**: なし（手動実行のみ）
   - **処理内容**: データリセット → テストデータ投入 → メニュー生成 → リマインダー送信の全フロー検証
   - **対象環境**: 検証環境（`ENV=development`）
   - **用途**: 新機能実装時のテストやシステム全体の動作確認
...
システム全体の動作を検証したい場合は、**Test Full Cycle** ワークフローを実行してください。

このワークフローは以下を自動実行します：

1. **データリセット** - 既存データを削除
2. **テストデータ投入** - サンプル実績データを投入
3. **献立生成** - AIで献立を生成
4. **リマインダー送信** - Slackに通知を送信

### 実行方法

1. GitHub リポジトリの **Actions** タブをクリック
2. 左メニューから **Test Full Cycle** を選択
3. **Run workflow** ドロップダウンをクリック
4. **reset_data** オプション（省略可、デフォルト: `all`）：
   - `all` - すべてのテーブルをリセット
...
**Notion 確認**:
- ✅ `Proposed_Dishes`: 21件の献立データ（3品 × 7日）
- ✅ `Raw_Actual_Input`: 4件のテストデータが「処理済み」に更新
- ✅ `Structured_Actual_History`: 構造化された実績データ
...
**重要**: このワークフローは `ENV=development` で実行されるため、本番の Notion/Slack には影響しません。テスト用のプライベートなデータベース・チャンネルでのテストを推奨します。
